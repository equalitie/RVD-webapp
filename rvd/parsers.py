...
# Given a list of strings of integers that should be indeces into an array,
...
def _make_indeces(indeces, array_len):
    for i in range(len(indeces)):
        try:
            v = int(indeces[i])
        except ValueError:
            indeces[i] = '{0} not a valid integer.'.format(indeces[i])
            continue
        if v < 0 or v >= array_len:
            indeces[i] = 'Index ({0}) must be between 1 and {1} inclusive'.format(
                v, array_len)
        else:
            indeces[i] = v - 1
    return indeces

# Using a list of indeces (or error strings) as generated by _make_indeces,
# take items from a given array with the specified indeces in order.
def _take(array, indeces):
    pulled = []
...
# A convenience function to convert a string listing indeces (e.g. "1, 2, 4")
...
def _from_indeces(ind_list, other_entities):
    indeces = _make_indeces(ind_list.split(','), len(other_entities))
    values = _take(other_entities, indeces)
    return values
...
def _action_h(parsed, wb):
    ...
    for i in range(len(parsed[ACTIONS])):
        ...
        action['state_bodies_approached'] = _from_indeces(
            action['state_bodies_approached'], parsed[STATE_AUTHORITIES])
        action['international_bodies_approached'] = _from_indeces(
            action['international_bodies_approached'], parsed[INTERNATIONAL_AUTHORITIES])
        action['events'] = _from_indeces(action['events'], parsed[EVENTS])
...
def _actor_h(parsed, wb):
    ...
    for i in range(len(parsed[ACTORS])):
        ...
        actor['organisations'] = _from_indeces(actor['organisations'], parsed[ORGANISATIONS])
        actor['professions'] = _from_indeces(actor['professions'], parsed[PROFESSIONS])
        actor['locations'] = _from_indeces(actor['locations'], parsed[LOCATIONS])
